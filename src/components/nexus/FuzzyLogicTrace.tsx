...
const getInputValueColor = (key: string, value: number | string): string => {
  if (typeof value === 'string') return 'text-nexus-cyan';
  
  // Different thresholds for different input types
  switch (key) {
    case 'queueLength':
      if (value <= 5) return 'text-nexus-open';
      if (value <= 15) return 'text-amber-400';
      return 'text-nexus-hold';
    case 'zoneOccupancy':
      if (value <= 40) return 'text-nexus-open';
      if (value <= 70) return 'text-amber-400';
      return 'text-nexus-hold';
    case 'dwellTimeAvg':
      if (value <= 30) return 'text-nexus-open';
      if (value <= 50) return 'text-amber-400';
      return 'text-nexus-hold';
    case 'arrivalRate':
      if (value <= 8) return 'text-nexus-open';
      if (value <= 15) return 'text-amber-400';
      return 'text-nexus-hold';
    case 'serviceRate':
      // Higher is better for service rate
      if (value >= 12) return 'text-nexus-open';
      if (value >= 6) return 'text-amber-400';
      return 'text-nexus-hold';
    default:
      return 'text-nexus-cyan';
  }
};

// Get background color for input value
const getInputBgColor = (key: string, value: number | string): string => {
  if (typeof value === 'string') return 'bg-nexus-cyan/10';
  
  switch (key) {
    case 'queueLength':
      if (value <= 5) return 'bg-nexus-open/10';
      if (value <= 15) return 'bg-amber-400/10';
      return 'bg-nexus-hold/10';
    case 'zoneOccupancy':
      if (value <= 40) return 'bg-nexus-open/10';
      if (value <= 70) return 'bg-amber-400/10';
      return 'bg-nexus-hold/10';
    case 'dwellTimeAvg':
      if (value <= 30) return 'bg-nexus-open/10';
      if (value <= 50) return 'bg-amber-400/10';
      return 'bg-nexus-hold/10';
    case 'arrivalRate':
      if (value <= 8) return 'bg-nexus-open/10';
      if (value <= 15) return 'bg-amber-400/10';
      return 'bg-nexus-hold/10';
    case 'serviceRate':
      if (value >= 12) return 'bg-nexus-open/10';
      if (value >= 6) return 'bg-amber-400/10';
      return 'bg-nexus-hold/10';
    default:
      return 'bg-nexus-cyan/10';
  }
};
...
export const FuzzyLogicTrace = ({ decisions, className }: FuzzyLogicTraceProps) => {
  ...
  const getActionIcon = (action: FuzzyLogicDecision['action'], size: 'sm' | 'md' = 'sm') => {
    ...
    switch (action) {
      case 'OPEN': return <DoorOpen className={cn(sizeClass, "text-nexus-open")} />;
      case 'CLOSE': return <DoorClosed className={cn(sizeClass, "text-nexus-hold")} />;
      case 'HOLD': return <Pause className={cn(sizeClass, "text-nexus-wait")} />;
    }
  };
  ...
  const getStatusIcon = (action: FuzzyLogicDecision['action']) => {
    switch (action) {
      case 'OPEN': return <CheckCircle className="h-3 w-3 text-nexus-open" />;
      case 'CLOSE': return <XCircle className="h-3 w-3 text-nexus-hold" />;
      case 'HOLD': return <AlertTriangle className="h-3 w-3 text-nexus-wait" />;
    }
  };
  ...
  const ConfidenceMeter = ({ confidence }: { confidence: number }) => {
    ...
    const getConfidenceColor = () => {
      if (percentage >= 85) return 'from-nexus-open to-emerald-400';
      if (percentage >= 60) return 'from-nexus-cyan to-blue-400';
    ...
    return (
      <div className="flex items-center gap-2">
        <Gauge className="h-4 w-4 text-nexus-cyan/70" />
        <div className="flex-1 min-w-[80px]">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[9px] font-mono text-muted-foreground uppercase">Certainty</span>
            <span className={cn(
              "text-[9px] font-bold font-mono",
              percentage >= 85 ? 'text-nexus-open' : percentage >= 60 ? 'text-nexus-cyan' : 'text-amber-400'
            )}>
              {getConfidenceLabel()}
            </span>
          </div>
  ...
  const DecisionFlowchart = ({ decision }: { decision: FuzzyLogicDecision }) => {
    ...
    return (
      <div className="bg-black/30 rounded-lg border border-nexus-cyan/20 p-3">
        <div className="flex items-center gap-1.5 mb-3">
          <Zap className="h-3.5 w-3.5 text-nexus-cyan" />
          <span className="text-[10px] font-mono font-bold text-nexus-cyan uppercase tracking-wider">Decision Path</span>
        </div>
        
        {/* Flowchart visualization */}
        <div className="flex items-center gap-1 text-[10px] font-mono overflow-x-auto pb-1">
          {/* Input node */}
          <div className="flex-shrink-0 px-2 py-1.5 bg-nexus-cyan/10 border border-nexus-cyan/30 rounded">
            <span className="text-nexus-cyan font-bold">INPUT</span>
          </div>
          
          <ChevronRight className="h-3 w-3 text-nexus-cyan/50 flex-shrink-0" />
          ...
          >
            <span className={cn("transition-colors duration-300", inputs.queueLength > 10 || inputs.zoneOccupancy > 60 ? "text-amber-400" : "text-nexus-open")}>
          ...
          <div 
            className={cn(
              ...
              action === 'OPEN' && "bg-nexus-open/10 border-nexus-open/30",
              action === 'CLOSE' && "bg-nexus-hold/10 border-nexus-hold/30",
              action === 'HOLD' && "bg-nexus-wait/10 border-nexus-wait/30"
            )}
          >
            {getActionIcon(action, 'sm')}
            <span className={cn(
              "font-bold transition-colors duration-300",
              action === 'OPEN' && "text-nexus-open",
              action === 'CLOSE' && "text-nexus-hold",
              action === 'HOLD' && "text-nexus-wait"
            )}>
              {action}
            </span>
          </div>
  ...
  const TriggeredRuleDisplay = ({ decision }: { decision: FuzzyLogicDecision }) => {
    ...
    return (
      <div 
        className="bg-gradient-to-r from-nexus-cyan/5 to-transparent border-l-2 border-nexus-cyan px-3 py-2 rounded-r"
      >
        <div className="flex items-center gap-1.5 mb-1">
          <Terminal className="h-3 w-3 text-nexus-cyan" />
          <span className="text-[9px] font-mono text-nexus-cyan uppercase tracking-wider">Triggered Rule</span>
        </div>
        <code className="text-xs font-mono text-foreground/90 block">
          ...
          <span className={cn(
            decision.action === 'OPEN' && "text-nexus-open",
            decision.action === 'CLOSE' && "text-nexus-hold",
            decision.action === 'HOLD' && "text-nexus-wait"
          )}>
            {rule.split('THEN ')[1]}
          </span>
        </code>
  ...
  const HistoryItem = ({ decision, index }: { decision: FuzzyLogicDecision; index: number }) => (
    <div 
      className="flex items-center gap-2 px-2 py-1.5 bg-black/20 rounded border border-white/5 hover:border-nexus-cyan/20 transition-colors"
    >
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-nexus-open opacity-75" style={{ animationDuration: '2s' }}></span>
      ...
      <div className="p-0">
        {decisions.length === 0 ? (
          ...
          <div className="divide-y divide-border/20">
              ...
              {currentDecision && (
                <div className="p-4 bg-nexus-cyan/5 w-full space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        currentDecision.action === 'OPEN' && "text-nexus-open",
                        currentDecision.action === 'CLOSE' && "text-nexus-hold",
                        currentDecision.action === 'HOLD' && "text-nexus-wait"
                      )}>
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 mb-2">
                      <Activity className="h-3.5 w-3.5 text-nexus-cyan/70" />
                      <span className="text-[10px] font-mono font-bold text-nexus-cyan/70 uppercase tracking-wider">Input Values</span>
                    </div>
                  ...
                  {/* Rules trace */}
                  <div className="space-y-2 pl-2 border-l-2 border-nexus-cyan/30 ml-1">
              ...
              {decisions.length > 4 && (
                <div className="max-h-32 overflow-y-auto scrollbar-thin bg-black/20">
                  {decisions.slice(4).map((decision) => (
                    <div key={decision.id} className="px-3 py-2 hover:bg-white/5 transition-colors border-l-2 border-transparent hover:border-nexus-cyan/30">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono text-muted-foreground">{format(decision.timestamp, 'HH:mm:ss')}</span>
                        {getStatusIcon(decision.action)}
                        <span className={cn(
